...
async function openaiAction(action) {
	...
	if (!conversation) forms.set(form, (conversation = []));
	...
	for (let element of elements) {
		...
		if (typeof content === "string") conversation.push({ role, content });
			...
			if (content.method) {
				...
				for (let i = 0; i < content[type].length; i++) {
					if (typeof content[type][i].content === "string")
						conversation.push({
							role,
							content: content[type][i].content
						});
					else
						conversation.push({
							role,
							content: JSON.stringify(content[type][i])
						});
				}
			} else
	...
	if (conversation.length) send(conversation, action);
}
